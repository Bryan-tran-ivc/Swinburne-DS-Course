...
def print_album_details(album):
    ...
    print("Genre : " + GENRE.get(album.genre, "Unknown"))
    print("Tracks: " + str(len(album.tracks)) + " tracks loaded.")
...
def update_album(albums):
    ...
    i = 0
    while i < len(albums):
        print(str(i + 1) + ": " + albums[i].title)
        i += 1

    album_id = input_functions.read_int_in_range("Enter Album ID to update: ", 1, len(albums))
    album    = albums[album_id - 1]

    print("\nWhat would you like to update?")
    print("1. Update Title")
    print("2. Update Genre")
    update_choice = input_functions.read_int_in_range("Enter choice: ", 1, 2)

    if update_choice == 1:
        album.title = input_functions.read_string("Enter new title: ")
        ...
        while g <= len(GENRE):
            print("  " + str(g) + ": " + GENRE[g])
            g += 1
        album.genre = input_functions.read_int_in_range("Enter new Genre ID: ", 1, len(GENRE))
...
def main():
    ...
    while not finished:
        ...
        choice = input_functions.read_int_in_range("Enter choice: ", 1, 5)
